...
- NVT : 원격지 시스템과 로컬 시스템의 버전이 상이할 경우 데이터 형식을 변환해 원활한 통신을 하게 하는 가상장치
...
### Telnet/SSH를 사용해 클라이언트 서버와 원격 서버 인증을 해서 FTP/SFTP로 파일을 전송함  
...
수정 후
```
$query = "INSERT INTO testdb (Title, Content, Company, UserName, Id, Date, Hit) 
            VALUES ('$title', '$content', '$company', '$username', '$id', now(), 0)";
```
